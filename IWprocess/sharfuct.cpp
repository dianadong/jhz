...
void printlist(entlist elist)
{
	...
	do
	{
		printf("%s\t%s\t%d\n", pk->pentityk,pk->pidcode,  pk->startsit);
		pk = pk->pnext;
	}while(pk != NULL);
}
///////////////////////////////////////////////////////////
...
void sourceqiefen(char ** psource,char separator,char *result)
{
	...
	s = NULL;
	s = strchr(*psource, separator);
	
	/*separesult = (char *)malloc(1 << 10);
	*separesult = NULL;
	*/
	if(s != NULL)
	{                       
		...
		{
             *result = 0;
			 strncpy(result, *psource, len);
			 *(result+len) = 0;
			/**separesult = 0;
			strncpy(separesult, *psource, len);
			*(separesult + len) = 0;
			result = separesult;*/
		}
		 else
	    {
			 printf("There are two neighbouring separator!");
             *psource = s +1;
		}
...
entlist * createuserlist(const char *pbuf, entlist *SL)
	{
		char * pa,* s,* temp,* pentity,* pidcode,* number,* jhtstr;
        int len,startsit,i;
       
 
        jhtstr = (char *)malloc(1<<14);
        *jhtstr = 0;
        pentity = (char *)malloc(1<<14);
        *pentity = 0;
        pidcode = (char *)malloc(1<<14);
        *pidcode = 0;
        number = (char *)malloc(1<<14);
        *number= 0;
   ...
   while(* pa != 0)
		{
		   ...
		   sourceqiefen(&temp,0x2C,pentity);//','
		   sourceqiefen(&temp,0x2C,pidcode);
		   //printf("partk=%s\n",partk);
		   sourceqiefen(&temp,0x2C,number);
		   startsit=atoi(number);
		   
		   ins_entList(SL,pentity, pidcode,startsit);
