...
siattlist SortInsert(siattlist *head, siattlist node)
{
	...
	p = *head;             //有序链表不为空  
	while (p->fqval < node->fqval && p != NULL)    //p指向的节点的学号比插入节点的学号小，并且它不等于NULL  
	{
...
//time: 2016-5-9
//////////////////////////////////////////////////////
int wfvhash2list(hashtable wsvhash, subnode * SL)
{
	long i;
	int znum = 0;
	if(wsvhash == NULL)
   ...
   while( *pa != 0 && strlen(pa) > 1)
	{
		 ...
		 //  printf("jhtstr=%s,strlen(jhtstr)=%d\n",jhtstr,strlen(jhtstr));
		  /* for(i=1;i<=strlen(jhtstr)+1;i++)
			   pa++;*/
		   /*s = NULL;
		   s = strchr(pa, 0x09);//'/t'
		   if(s != NULL)
...
SIATTBNODE * LocateNode(const char *key ,const siattlist intentL)
{
	...
	while(srclist != NULL )
    {
       if(strcmp(key,srclist->pword)==0)
             //if(*key== *(srclist->pword))
        {

           // printf("key is %s\n",key);
            //printf("srclist->pword is %s\n",srclist->pword);
         break;
        }
...
///////////////////////////////////////////////////////////////
///function:   本次意图参选分析，采用建堆（找出Top K）。
//parameter:
//author: wuxiaoqi
//time: 2016-5-9
///////////////////////////////////////////////////////////////
